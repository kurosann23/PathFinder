...
export function StudentAppointmentPage() {
  ...
  // Pre-filter appointments for the render logic
              ...
              >
                ...
                {teachers.map((teacher) => (
                  <option key={teacher.id} value={teacher.id}>
                    {teacher.full_name || 'Teacher'} {teacher.email ? `(${teacher.email})` : ''}
                  </option>
                ))}
              </select>
            </div>
                            ...
                            <div className="text-sm font-medium text-slate-200 mb-1">
                              ...
                              {appointment.teacher_email && (
                                <span className="text-slate-400 font-normal ml-1">
                                  (<a href={`mailto:${appointment.teacher_email}`} className="hover:text-blue-400 hover:underline">{appointment.teacher_email}</a>)
                                </span>
                              )}
                            </div>
                            {appointment.reason && (
                          ...
                          >
                            <div className="flex items-center gap-3 min-w-0 flex-1">
                              ...
                              <span className={cn(
                                ...
                                {appointment.teacher_name || 'Teacher'} {appointment.teacher_email ? `(${appointment.teacher_email})` : ''}
                              </span>
                              <span className={cn(
                                "text-xs shrink-0",
                                isLight ? "text-slate-600" : "text-slate-400"
              ...
              {rejectedAppointments.length > 0 && (
                <Card>
                  <div className="mb-4">
                    ...
                    <p className="mt-1 text-xs text-slate-400">
                      {rejectedAppointments.length} rejected appointment{rejectedAppointments.length !== 1 ? 's' : ''}
                    </p>
                  </div>
                  <div className="space-y-2">
